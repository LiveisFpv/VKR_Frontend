...
Основные модули: поиск статей с историей «чатов», добавление статьи, личный кабинет, админ‑панель пользователей, модераторская панель, тёмная/светлая тема, простая i18n (en/ru), тост‑уведомления и диалоги подтверждения.
...
- `VITE_ALIB_API_URL` — базовый URL API поиска и добавления статей.
...
- `src/stores/` — Pinia‑хранилища:
  ...
  - `chatStore.ts` — история «чатов» поиска, сообщения и результаты.
...
- `src/api/` — слои API на Axios:
  ...
  - `api/useAlibApi.ts` — методы поиска и добавления статьи (`/ai/search/papers`, `/ai/paper/add`).
- `src/i18n.ts` — простая i18n без внешних библиотек: реактивная локаль, словарь сообщений, хелпер `t()` и `useI18n()`.
- `src/components/` — UI‑компоненты: верхняя панель `UpTab`, левая панель/история `LeftTab`, поиск `MainChat`, тост‑центр `ToastCenter`, диалог подтверждения `ConfirmDialog`, футер `FooterTab`.
...
- Поиск: `MainChat.vue` использует `AlibApi.search(text)` и отображает результаты по «ходам» (история сообщений). Предпросмотр карточки, ссылки на PDF/страницу, OpenAccess‑мета.
